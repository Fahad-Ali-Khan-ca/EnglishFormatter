...
auto test_env(char *apiKeyCStr, char *apiUrlCStr) {
  if (apiKeyCStr == nullptr) {
    std::cerr << "Error: API_KEY is not set. Please set it in the .env file or "
                 "as an environment variable."
              << std::endl;
    return false;
  } else if (apiUrlCStr == nullptr) {
    std::cerr << "Error: API_URL is not set. Please set it in the .env file or "
                 "as an environment variable."
              << std::endl;
    return false;
  } else {
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--version" || arg == "-v") {
      ...
      showTokenUsage =
          true;  // Set the token usage flag to true if this option is passed
    } else if (arg == "--model" || arg == "-m") {
      if (i + 1 < argc) {
        std::string old_model = model;
        model = argv[++i];  // Increment i to skip over the value
        std::cout << "Model changed from " << old_model << " to " << model
                  << "." << std::endl;
      } else {
        std::cerr << "Error: The option '" << arg
                  << "' requires a model name as an argument." << std::endl;
        return 1;  // Exit with error code
      }
    } else if (arg == "--output" || arg == "-o") {
      if (i + 1 < argc) {
        output_name = argv[++i];  // Increment i to skip over the value
        std::cout << "Output files will now be saved under [name]"
                  << output_name << std::endl;
      } else {
        std::cerr << "Error: The option '" << arg
                  << "' requires an output name as an argument." << std::endl;
        return 1;  // Exit with error code
      }
    } else {
  ...
  // Define the menu items this should be done dynamically by someone who was
  // administrator priviledges potential user input for more diversity
  std::vector<std::string> menuItems = {"Format document", "Summarize document",
                                        "Paraphrase document", "Exit"};
