...
void eng_format::convert_file(std::string filename, std::string action) {
  try {
    ...
    std::string prompt =
        format("Can you please {0} the text and provide the {0} text only. {1}",
               action, content);
    ...
    std::cerr << "Error converting file " << filename << ": " << e.what()
              << std::endl;
    throw;
  }
}
...
std::string eng_format::get_token_info(const std::string& response) {
  try {
    ...
    throw std::runtime_error("Failed to parse token info from response: " +
                             std::string(e.what()));
  }
}
